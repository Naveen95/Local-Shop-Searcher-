		...
		<div class="container">
			<div class="row">
					...
					<div class="contact-form margint60"><!-- Contact Form -->
						...
						<h1><center>Your Current Details</center></h1>

						<center>
						@foreach($user_details as $user_detail)

						<h3>NAME : {{$user_detail->name}}<br>
						PHONE : {{$user_detail->phone}}<br>
						EMAIL : {{$user_detail->email}}<br>
						CITY : {{$user_detail->city}}<br>
						AREA : {{$user_detail->area}}<br></h3>
					</center>

						@endforeach
						<center><h2>Update Your Phone , city and Area below</h2></center></br>
						<form action="{{url('/personalinfo')}}" method="POST">
							{{csrf_field()}}
							<div class="col-lg-12 col-sm-8 center-block">
							
							
							<input type="text" placeholder="phone" name="phone" >
							<input type="email" placeholder="E-Mail" name="email" >
							<input type="text" placeholder="City" name="city" >
							<input type="text" placeholder="area" name="area" ><br>
							
							
							<input class="pull-left margint10" type="submit" value="SUBMIT">
						</div>	
						</form>
					</div>
				</div>
